...
export default function CoffeePomodoroApp() {
  ...
  const [spotifyUrl, setSpotifyUrl] = useState(() => {
    try {
      return localStorage.getItem("coffee-pomodoro-spotify") ?? "";
    } catch {
      return "";
    }
  });
  ...
  useEffect(() => {
    try {
      if (spotifyUrl) localStorage.setItem("coffee-pomodoro-spotify", spotifyUrl);
    } catch {
      /* ignore */
    }
  }, [spotifyUrl]);
  ...
  return (
    <div className="min-h-screen bg-[#f5e6d3] text-[#2c1a18] p-4 md:p-8">
      ...
      <div className="relative max-w-5xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="rounded-3xl shadow-cup bg-[#fff8f0] border-2 border-[#d4b896] p-6 md:p-8 w-full">
              ...
              <div className="flex flex-wrap gap-2 justify-center mt-6">
                <button
                  type="button"
                  onClick={startTimer}
                  className="px-5 py-2.5 rounded-xl bg-[#4b2e2b] text-[#fff8f0] font-semibold hover:bg-[#3d2522] active:scale-[0.98] transition shadow-md"
                >
                  Başlat
                </button>
                <button
                  type="button"
                  onClick={pauseTimer}
                  className="px-5 py-2.5 rounded-xl bg-[#e8d4bc] text-[#2c1a18] font-semibold hover:bg-[#d4b896] active:scale-[0.98] transition"
                >
                  Duraklat
                </button>
                <button
                  type="button"
                  onClick={resetTimer}
                  className="px-5 py-2.5 rounded-xl border-2 border-[#9c7355] text-[#4b2e2b] font-semibold hover:bg-[#f5e6d3] active:scale-[0.98] transition"
                >
                  Sıfırla
                </button>
              </div>
          ...
          >
            <div className="rounded-3xl shadow-xl bg-[#fff8f0] border-2 border-[#d4b896] overflow-hidden h-full p-6 md:p-8">
              <h2 className="font-display text-xl font-bold text-[#2c1a18] mb-5">
                Ayarlar
              </h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-bold text-[#5c3d38] mb-1.5">
                    Çalışma süresi (dk)
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) => {
                      ...
                      if (!isNaN(v) && v >= 1 && v <= 120) setWorkTime(v);
                    }}
                    className="w-full px-4 py-3 rounded-xl border-2 border-[#d4b896] bg-white text-[#2c1a18] font-medium focus:border-[#7d5a42] focus:ring-2 focus:ring-[#e8d4bc] outline-none transition"
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-[#5c3d38] mb-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const v = Number(e.target.value);
                      if (!isNaN(v) && v >= 1 && v <= 60) setBreakTime(v);
                    }}
                    className="w-full px-4 py-3 rounded-xl border-2 border-[#d4b896] bg-white text-[#2c1a18] font-medium focus:border-[#7d5a42] focus:ring-2 focus:ring-[#e8d4bc] outline-none transition"
                  />
                </div>
              </div>
              <p className="mt-4 text-xs font-medium text-[#7d5a42]">
                Değişiklikler yeni turda uygulanır.
              </p>
            </div>
          </motion.div>
          ...
          >
            <div className="rounded-3xl shadow-xl bg-[#fff8f0] border-2 border-[#d4b896] overflow-hidden h-full p-6 md:p-8">
              <h2 className="font-display text-xl font-bold text-[#2c1a18] mb-2">
                Spotify
              </h2>
              <p className="text-sm font-medium text-[#5c3d38] mb-4">
                Embed linkini yapıştır.
              </p>
              <input
                type="url"
                ...
                className="w-full px-4 py-3 rounded-xl border-2 border-[#d4b896] bg-white text-[#2c1a18] placeholder-[#9c7355] font-medium focus:border-[#7d5a42] focus:ring-2 focus:ring-[#e8d4bc] outline-none transition"
              />
              {spotifyUrl && (
                <div className="mt-4 rounded-xl overflow-hidden bg-[#2c1a18]/05">
                  <iframe
                    src={spotifyUrl}
                    width="100%"
                    height="152"
        ...
        >
          <p className="font-display text-xl font-semibold text-[#4b2e2b]">
            Kahveni al, odaklan ve üret
          </p>
        </motion.footer>
      </div>
    </div>
